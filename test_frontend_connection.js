...
function makeRequest(method, endpoint, body = null, token = null, isPublic = false) {
  return new Promise((resolve, reject) => {
    const fullUrl = isPublic ? `http://localhost:3001${endpoint}` : API_BASE_URL + endpoint;
...
async function testConnection() {
  ...
  try {
    ...
    const healthCheck = await makeRequest('GET', '/health', null, null, true);
    ...
    if (classes.status === 200 && classes.data.data && classes.data.data.length > 0) {
      console.log('✅ Clases obtenidas: ' + classes.data.data.length);
      const firstClass = classes.data.data[0];
      console.log('   Primera clase: ' + (firstClass.subject || firstClass.name || 'N/A') + '\n');
    } else {
      console.log('❌ Error obteniendo clases: ' + (classes.data.message || 'Sin detalles') + '\n');
    }
    ...
    if (stats.status === 200 && stats.data.data) {
      ...
      console.log('   Total de clases: ' + (stats.data.data.classes || stats.data.data.total_classes || 0));
      console.log('   Total de profesores: ' + (stats.data.data.teachers || stats.data.data.total_teachers || 'N/A') + '\n');
    } else {
      console.log('⚠️  Info: Teachers no pueden ver estadísticas del dashboard (403 Forbidden)');
      console.log('   Intentando login admin para verificar estadísticas globales...\n');
    }
    ...
    if (adminLogin.status === 200) {
      ...
      // Test 7b: Get Admin Statistics
      console.log('📊 Test 7b: Obtener Estadísticas de Admin');
      const adminStats = await makeRequest('GET', '/statistics/dashboard', null, adminToken);
      if (adminStats.status === 200 && adminStats.data.data) {
        console.log('✅ Estadísticas globales obtenidas');
        console.log('   Total de entidades: ' + (adminStats.data.data.entities || 'N/A'));
        console.log('   Total de clases: ' + (adminStats.data.data.classes || 0));
        console.log('   Total de profesores: ' + (adminStats.data.data.teachers || 0) + '\n');
      } else {
        console.log('❌ Error obteniendo estadísticas de admin\n');
      }
    } else {
