...
exports.getAttendanceSessions = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const countResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT a.date) as total FROM attendance a ${whereClause}`, params);
    const total = parseInt(countResult.rows[0].total);
    const offset = (parseInt(page) - 1) * parseInt(limit);
    // Obtener fechas de sesiones (agrupadas por día)
    const sessionsResult = await (0, connection_1.query)(`SELECT DISTINCT a.date as session_date, 
...
exports.getAttendanceSession = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const recordsResult = await (0, connection_1.query)(`SELECT a.id, a.class_id, a.date, a.student_id, a.status, a.notes, a.recorded_by, a.created_at,
...
exports.createAttendanceSession = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!class_id || !date || !student_id || !status) {
        throw (0, errorHandler_1.createError)('class_id, date, student_id y status son requeridos', 400);
    }
    ...
    const classData = classCheck.rows[0];
    if (req.user?.role === 'admin_entity' && classData.entity_id !== req.user.entityId) {
        throw (0, errorHandler_1.createError)('No tienes permiso', 403);
    }
    if (req.user?.role === 'teacher' && classData.teacher_id !== req.user.id) {
        throw (0, errorHandler_1.createError)('No tienes permiso', 403);
    }
    // Verificar si ya existe un registro para este estudiante en esa fecha
    const existingCheck = await (0, connection_1.query)(`SELECT id FROM attendance 
     WHERE class_id = $1 AND student_id = $2 AND DATE(date) = $3`, [class_id, student_id, date]);
    let result;
    if (existingCheck.rows.length > 0) {
        // Actualizar
        result = await (0, connection_1.query)(`UPDATE attendance 
       SET status = $1, notes = $2, updated_at = NOW()
       WHERE id = $3
       RETURNING id, class_id, date, student_id, status, notes, recorded_by, created_at`, [status, notes || null, existingCheck.rows[0].id]);
    }
    else {
        // Insertar
        result = await (0, connection_1.query)(`INSERT INTO attendance (class_id, student_id, date, status, notes, recorded_by)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, class_id, date, student_id, status, notes, recorded_by, created_at`, [class_id, student_id, date, status, notes || null, req.user?.id]);
    }
    const response = {
        success: true,
        data: result.rows[0],
...
exports.getAttendanceRecords = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const recordsResult = await (0, connection_1.query)(`SELECT a.id, a.class_id, a.date, a.student_id, a.status, a.notes, a.recorded_by, a.created_at,
     ...
     JOIN students s ON a.student_id = s.id
     ${whereClause}
     ORDER BY a.date DESC, s.first_name, s.last_name`, params);
    const response = {
        success: true,
        data: recordsResult.rows,
        message: 'Registros de asistencia obtenidos exitosamente',
...
exports.recordAttendance = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!class_id || !date || !records || !Array.isArray(records)) {
        throw (0, errorHandler_1.createError)('class_id, date y records (array) son requeridos', 400);
    }
    // Validar permisos
    const classCheck = await (0, connection_1.query)(`SELECT teacher_id, entity_id FROM classes WHERE id = $1`, [class_id]);
    if (classCheck.rows.length === 0) {
...
exports.updateAttendanceRecord = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const result = await (0, connection_1.query)(`UPDATE attendance
     ...
     WHERE id = $3
     RETURNING id, class_id, date, student_id, status, notes, recorded_by, created_at`, [status || null, notes || null, record_id]);
    const response = {
        success: true,
        data: result.rows[0],
...
//# sourceMappingURL=attendance.controller.NEW.js.map