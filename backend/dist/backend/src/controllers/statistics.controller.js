...
exports.getEntityStatistics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    if (req.user?.role === 'admin_entity') {
        ...
        const teachersResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT c.teacher_id) as total FROM classes c WHERE c.entity_id = $1`, [entityId]);
       ...
       WHERE c.entity_id = $1`, [entityId]);
        ...
        const teachersResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT c.teacher_id) as total FROM classes c`);
...
exports.getClassStatistics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const studentsResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT e.student_id) as total
     FROM enrollments e
     JOIN sections s ON e.section_id = s.id
     WHERE s.id = (SELECT section_id FROM classes WHERE id = $1)`, [class_id]);
    // Promedio de calificaciones
    ...
    const assessmentsResult = await (0, connection_1.query)(`SELECT COUNT(*) as total FROM assessments WHERE class_id = $1`, [class_id]);
...
exports.getStudentStatistics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!class_id || !student_id) {
        throw (0, errorHandler_1.createError)('class_id y student_id son requeridos', 400);
    }
    // Validar que el usuario puede acceder a esta clase
    const classCheck = await (0, connection_1.query)(`SELECT entity_id, teacher_id FROM classes WHERE id = $1`, [class_id]);
    if (classCheck.rows.length === 0) {
    ...
    const studentResult = await (0, connection_1.query)(`SELECT id, first_name, last_name, identification_number FROM students WHERE id = $1`, [student_id]);
    ...
    const gradesResult = await (0, connection_1.query)(`SELECT 
     ...
     FROM grades_records gr
     JOIN assessments a ON gr.assessment_id = a.id
     WHERE a.class_id = $1 AND gr.student_id = $2`, [class_id, student_id]);
    // Asistencia
    const attendanceResult = await (0, connection_1.query)(`SELECT 
      COUNT(a.id) as total_records,
      SUM(CASE WHEN a.status = 'present' THEN 1 ELSE 0 END) as present,
      ...
      SUM(CASE WHEN a.status = 'late' THEN 1 ELSE 0 END) as late
     FROM attendance a
     WHERE a.class_id = $1 AND a.student_id = $2`, [class_id, student_id]);
    const grades = gradesResult.rows[0];
    const attendance = attendanceResult.rows[0];
    const data = {
        student: {
