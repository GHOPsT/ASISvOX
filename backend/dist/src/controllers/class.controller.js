...
exports.createClass = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!subjectId || !sectionId || !academicYearId) {
        throw (0, errorHandler_1.createError)('subjectId, sectionId y academicYearId son requeridos', 400);
    }
    ...
    const sectionResult = await (0, connection_1.query)(`SELECT sec.id, sec.grade_id 
     FROM sections sec
     JOIN grades g ON sec.grade_id = g.id
     WHERE sec.id = $1 AND sec.academic_year_id = $2`, [sectionId, academicYearId]);
    if (sectionResult.rows.length === 0) {
        throw (0, errorHandler_1.createError)('Sección no encontrada', 404);
    }
    // Validar que la materia existe
    const subjectResult = await (0, connection_1.query)('SELECT id FROM subjects WHERE id = $1 AND is_active = true', [subjectId]);
    if (subjectResult.rows.length === 0) {
        throw (0, errorHandler_1.createError)('Materia no encontrada o no está activa', 404);
    }
    // Validar que el año académico existe
    const yearResult = await (0, connection_1.query)('SELECT id FROM academic_years WHERE id = $1', [academicYearId]);
    ...
    try {
       ...
       RETURNING id, entity_id, section_id, subject_id, teacher_id, academic_year_id, classroom, weeks_duration, is_active, created_at, updated_at`, [sectionId, subjectId, req.user.id, academicYearId, req.user.entityId, classroom || null, weeksValue]);
