...
exports.getEntityStats = exports.deactivateEntity = exports.activateEntity = exports.deleteEntity = exports.updateEntity = exports.createEntity = exports.getEntityById = exports.getEntities = void 0;
...
const findEntityById = async (entityId) => {
    const result = await (0, connection_1.query)(`SELECT id, name, code, address, image_url, representative_name, 
...
const mapEntityToResponse = (dbEntity) => {
    return {
        id: dbEntity.id,
        name: dbEntity.name,
        code: dbEntity.code,
...
exports.getEntities = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede ver todas las entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede ver las entidades', 403);
    }
  ...
  `;
    ...
    const entitiesResult = await (0, connection_1.query)(entitiesQuery, params);
...
exports.getEntityById = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!entity) {
        throw (0, errorHandler_1.createError)('Entidad no encontrada', 404);
    }
...
exports.createEntity = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede crear entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede crear entidades', 403);
    }
    const { name, code, address, imageUrl, representativeName, representativePhone, representativeEmail, institutionalPhone, institutionalAddress, institutionalEmail, } = req.body;
    // Validaciones básicas
    if (!name || !code) {
        throw (0, errorHandler_1.createError)('name y code son requeridos', 400);
    }
    // Validar que el código sea único
    const existingEntity = await (0, connection_1.query)('SELECT id FROM entities WHERE code = $1', [code]);
    if (existingEntity.rows.length > 0) {
        throw (0, errorHandler_1.createError)('Ya existe una entidad con este código', 409);
    }
    try {
        const createResult = await (0, connection_1.query)(`INSERT INTO entities (
...
exports.updateEntity = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede actualizar entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede actualizar entidades', 403);
    }
    const { id } = req.params;
    const { name, code, address, imageUrl, representativeName, representativePhone, representativeEmail, institutionalPhone, institutionalAddress, institutionalEmail, isActive, } = req.body;
    const entity = await findEntityById(id);
    if (!entity) {
        throw (0, errorHandler_1.createError)('Entidad no encontrada', 404);
    }
    ...
    values.push(id);
    const updateResult = await (0, connection_1.query)(`UPDATE entities 
     SET ${updates.join(', ')}
     WHERE id = $${paramCount}
     RETURNING id, name, code, address, image_url, representative_name, 
               representative_phone, representative_email, institutional_phone,
...
exports.deleteEntity = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede eliminar entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede eliminar entidades', 403);
    }
    const { id } = req.params;
    const entity = await findEntityById(id);
    if (!entity) {
    ...
    const usersResult = await (0, connection_1.query)('SELECT COUNT(id) as count FROM users WHERE entity_id = $1', [id]);
    const usersCount = parseInt(usersResult.rows[0].count);
    if (usersCount > 0) {
        throw (0, errorHandler_1.createError)(`No se puede eliminar la entidad: tiene ${usersCount} usuarios asociados`, 409);
    }
    await (0, connection_1.query)('DELETE FROM entities WHERE id = $1', [id]);
    const response = {
        success: true,
...
exports.activateEntity = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede activar entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede activar entidades', 403);
    }
    const { id } = req.params;
    const entity = await findEntityById(id);
    if (!entity) {
    ...
    const updateResult = await (0, connection_1.query)(`UPDATE entities SET is_active = true, updated_at = CURRENT_TIMESTAMP 
     WHERE id = $1
     RETURNING id, name, code, address, image_url, representative_name, 
               representative_phone, representative_email, institutional_phone,
               institutional_address, institutional_email, is_active, 
...
exports.deactivateEntity = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede desactivar entidades
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede desactivar entidades', 403);
    }
    const { id } = req.params;
    const entity = await findEntityById(id);
    if (!entity) {
    ...
    const updateResult = await (0, connection_1.query)(`UPDATE entities SET is_active = false, updated_at = CURRENT_TIMESTAMP 
     WHERE id = $1
     RETURNING id, name, code, address, image_url, representative_name, 
               representative_phone, representative_email, institutional_phone,
               institutional_address, institutional_email, is_active, 
...
exports.getEntityStats = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    // REGLA: Solo admin_general puede ver estadísticas
    if (req.user?.role !== 'admin_general') {
        throw (0, errorHandler_1.createError)('Solo admin_general puede ver las estadísticas', 403);
    }
    const { id } = req.params;
    const entity = await findEntityById(id);
    if (!entity) {
    ...
    const usersStatsResult = await (0, connection_1.query)(`SELECT role, COUNT(id) as count 
     FROM users 
     WHERE entity_id = $1 
     GROUP BY role`, [id]);
    // Contar clases
    const classesResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT c.id) as count 
     FROM classes c
     WHERE c.entity_id = $1`, [id]);
    // Contar estudiantes por entidad
    const studentsResult = await (0, connection_1.query)(`SELECT COUNT(DISTINCT s.id) as count
     FROM students s
    ...
    const gradesStatsResult = await (0, connection_1.query)(`SELECT AVG(gr.score) as average_grade, 
     ...
     WHERE e.status = 'active'`, []);
    const usersByRole = {};
    usersStatsResult.rows.forEach((row) => {
