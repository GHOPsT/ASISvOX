...
export const getAttendanceSessions = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const countResult = await query(
    `SELECT COUNT(DISTINCT DATE(a.date)) as total FROM attendance a ${whereClause}`,
    params
  );
...
export const getAttendanceSession = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const sessionResult = await query(
    `SELECT a.*, c.teacher_id, c.entity_id
     FROM attendance a
  ...
  if (req.user?.role === 'admin_entity' && session.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para acceder a esta sesión', 403);
  }
  ...
  const recordsResult = await query(
    `SELECT a.id, a.class_id, a.student_id, a.status, a.notes, a.date, a.created_at,
...
export const createAttendanceSession = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const classCheck = await query(
    `SELECT teacher_id, entity_id, section_id FROM classes WHERE id = $1`,
    [class_id]
  );
  ...
  if (req.user?.role === 'admin_entity' && classData.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para crear sesión en esta clase', 403);
  }
  if (req.user?.role === 'teacher' && classData.teacher_id !== req.user.id) {
    throw createError('No tienes permiso para crear sesión en esta clase', 403);
  }
  ...
  for (const student of students) {
    const result = await query(
      `INSERT INTO attendance (class_id, student_id, date, status, recorded_by)
       ...
       RETURNING id, class_id, student_id, date, status, created_at`,
      [class_id, student.id, date, req.user?.id]
    );
...
export const getAttendanceRecords = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (req.user?.role === 'admin_entity' && classData.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para acceder a estos registros', 403);
  }
  if (req.user?.role === 'teacher' && classData.teacher_id !== req.user.id) {
    throw createError('No tienes permiso para acceder a estos registros', 403);
  }
  ...
  const params: any[] = [class_id, date];
  let paramIndex = 3;
...
export const recordAttendance = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!class_id || !student_id || !date || !status) {
    throw createError('class_id, student_id, date y status son requeridos', 400);
  }
  ...
  if (req.user?.role === 'admin_entity' && classData.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para registrar asistencia', 403);
  }
  if (req.user?.role === 'teacher' && classData.teacher_id !== req.user.id) {
    throw createError('No tienes permiso para registrar asistencia', 403);
  }
...
export const updateAttendanceRecord = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (req.user?.role === 'admin_entity' && record.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para actualizar este registro', 403);
  }
...
export const deleteAttendanceRecord = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { recordId } = req.params;
  ...
  if (req.user?.role === 'admin_entity' && record.entity_id !== req.user.entityId) {
    throw createError('No tienes permiso para eliminar este registro', 403);
  }
  if (req.user?.role === 'teacher' && record.teacher_id !== req.user.id) {
    throw createError('No tienes permiso para eliminar este registro', 403);
  }

  await query(`DELETE FROM attendance WHERE id = $1`, [recordId]);
